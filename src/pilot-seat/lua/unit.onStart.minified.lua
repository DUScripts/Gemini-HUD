...
<text x="%f" y="%f" font-size="11px" fill="yellow">%s</text>
      ...
      .warp-scan {
         ...
         background: #ff3a56;
                        ...
                        </div>]]end end end;local cL=0;local cM=0;local cN=0;if activeRadar.isConstructIdentified(aW)==1 and size~=""then local cE=activeRadar.getConstructName(aW)local cG=math.floor(activeRadar.getConstructDistance(aW))if cG>=1000 then cG=''..string.format('%0.1f',cG/1000)..' km ('..string.format('%0.2f',cG/200000)..' su)'else cG=''..cG..' m'end;local cO=tostring(aW):sub(-3)local cP=''..cO..' '..cE..''isILock=true;cL=math.floor(activeRadar.getConstructSpeed(aW)*3.6)if activeRadar.getTargetId(aW)==aW then ct=ct..[[
                     <div class="table-row2 thS">
                     <div class="table-cellS">
                     ]]..'['..size..'] '..cP..[[ <speedcolor> ]]..cL..[[ km/h</speedcolor><br><distcolor>]]..cG..[[</distcolor>
                     </div>
                     </div>]]else ct=ct..[[
            ...
            </div>]]if GHUD_show_echoes==true then local cS=''for cC,aW in pairs(radarDynamicData)do cS=cS..'<span style="left:'..cC..'px;height:'..aW..'px;"></span>'end;local cT=''for cC,aW in pairs(radarStaticData)do cT=cT..'<span style="left:'..cC..'px;height:'..aW..'px;"></span>'end;local htmlRadar=htmlRadar..[[
               <div class="radar-widget">
               <div class="d-widget">]]..cS..[[</div>
               <div class="s-widget">]]..cT..[[</div>
               <div class="labels">
      ...
      .warp-scan {
      ...
      </style>]]function zeroConvertToWorldCoordinates(d,system)local e=" *([+-]?%d+%.?%d*e?[+-]?%d*)"local f="::pos{"..e..","..e..","..e..","..e..","..e.."}"local g,h,i,j,k=string.match(d,f)if g==nil or h==nil or i==nil or j==nil or k==nil then system.print("Invalid POS!")return vec3()end;if g=="0"and h=="0"then return vec3(i,j,k)end;j=math.rad(j)i=math.rad(i)local l=atlas[tonumber(g)][tonumber(h)]local m=math.cos(i)local n=vec3(m*math.cos(j),m*math.sin(j),math.sin(i))return vec3(l.center)+(l.radius+k)*n end;function getPipeD(system)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then local aD=""local length1=-700*200000;local cU=800*200000;local cV=pos1;local cW=pos2;local pos111=zeroConvertToWorldCoordinates(cV,system)local pos222=zeroConvertToWorldCoordinates(cW,system)local DestinationCenter=vectorLengthen(pos111,pos222,length1)local DepartureCenter=vectorLengthen(pos111,pos222,cU)local cX=vec3(construct.getWorldPosition())local x=(DestinationCenter-DepartureCenter):normalize()local z=(cX-DepartureCenter):dot(x)/x:dot(x)if z<=0.then return(cX-DepartureCenter):len()elseif z>=(DestinationCenter-DepartureCenter):len()then return(cX-DestinationCenter):len()end;local A=DepartureCenter+z*x;local J=(A-cX):len()if J<1000 then aD=""..string.format("%0.0f",J).." m"elseif J<100000 then aD=""..string.format("%0.1f",J/1000).." km"else aD=""..string.format("%0.2f",J/200000).." su"end;return aD end end;function getPipeW(system)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;local length1=-700*200000;local cU=800*200000;local cV=pos1;local cW=pos2;local pos111=zeroConvertToWorldCoordinates(cV,system)local pos222=zeroConvertToWorldCoordinates(cW,system)local DestinationCenter=vectorLengthen(pos111,pos222,length1)local DepartureCenter=vectorLengthen(pos111,pos222,cU)local cX=vec3(construct.getWorldPosition())local x=(DestinationCenter-DepartureCenter):normalize()local z=(cX-DepartureCenter):dot(x)/x:dot(x)if z<=0.then return(cX-DepartureCenter):len()elseif z>=(DestinationCenter-DepartureCenter):len()then return(cX-DestinationCenter):len()end;local A=DepartureCenter+z*x;local cY="::pos{0,0,"..math.floor(A.x)..","..math.floor(A.y)..","..math.floor(A.z).."}"system.print("Pipe center")system.setWaypoint(cY)end end;function getPos4Vector(cZ)return"::pos{0,0,"..vec3(cZ).x..","..vec3(cZ).y..","..vec3(cZ).z.."}"end;function makeVector(as,at)local au=vec3(at).x-vec3(as).x;local y=vec3(at).y-vec3(as).y;local av=vec3(at).z-vec3(as).z;return vec3(au,y,av)end;function UTC()local c_=curTime-timeZone*3600;return c_ end;function UTCscaner(system)local c_=system.getArkTime()-timeZone*3600;return c_ end;function vectorLengthen(as,at,aw)local ax=makeVector(as,at)local ay=vec3(ax):len()local az=ay+aw;local aA=az/ay;local aB=ax*aA;local au=vec3(as).x+vec3(aB).x;local y=vec3(as).y+vec3(aB).y;local av=vec3(as).z+vec3(aB).z;local aC=vec3(au,y,av)return aC end;function start(unit,system,text)pos1time=0;pos2time=0;tspeed=0;tspeed1=0;mmode=true;deg2rad=math.pi/180;rad2deg=180/math.pi;ms2kmh=3600/1000;kmh2ms=1000/3600;showMarker=true;if GHUD_export_mode==true then system.print("---------------")system.print("The export mode is enabled ALT+G")else system.print("---------------")system.print("The export mode is disabled ALT+G")end;SU=10;calcTargetSpeed=targetSpeed/3.6;meterMarker=0;if databank_1.getStringValue(1)~=""and databank_1.getFloatValue(2)~=0 and databank_1.getStringValue(3)~=""and databank_1.getFloatValue(4)~=0 then system.print("Coordinates from DB are used!")pos1=databank_1.getStringValue(1)pos2=databank_1.getStringValue(3)pos1time=databank_1.getFloatValue(2)pos2time=databank_1.getFloatValue(4)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local d0=pos11:dist(pos22)local d1=pos2time-pos1time;tspeed=d0/d1;tspeed1=math.floor(d0/d1*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew 20 km "..lengthSU1 .." su, speed "..tspeed1 .." km/h")unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)else databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;system.print("Coordinates are missing set new or export")end end;function inTEXT(unit,system,text)if pos1~=0 and string.find(text,"::pos")and not string.find(text,"m::pos")and pos2==0 and GHUD_export_mode==false then pos2=text;databank_1.setStringValue(3,pos2)pos2time=math.floor(system.getUtcTime())databank_1.setFloatValue(4,pos2time)system.print(text.." pos2 saved")pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)local d0=pos11:dist(pos22)local d1=pos2time-pos1time;tspeed=d0/d1;tspeed1=math.floor(d0/d1*3.6)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;meterMarker1=meterMarker1+tspeed*4;length1=meterMarker1;resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)meterMarker=meterMarker+calcTargetSpeed*4;length=meterMarker;resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.print("---------------")system.print("The coordinates are set manually!")posExport1=databank_1.getStringValue(1)posExport2=databank_1.getStringValue(3)timeExport1=math.floor(databank_1.getFloatValue(2))timeExport2=math.floor(databank_1.getFloatValue(4))system.print("The coordinates were exported to screen")screen_1.setCenteredText(posExport1 .."/"..timeExport1 .."/"..posExport2 .."/"..timeExport2)system.print("Target speed: "..tspeed1 .." km/h")unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if pos1==0 and string.find(text,"::pos")and not string.find(text,"m::pos")and GHUD_export_mode==false then pos1=text;databank_1.setStringValue(1,pos1)pos1time=math.floor(system.getUtcTime())databank_1.setFloatValue(2,pos1time)system.print(text.." pos1 saved")end;if text=="n"then pp1=''unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;system.print("---------------")system.print("Coordinates have been deleted, set new coordinates")end;if GHUD_export_mode==true and string.find(text,"/")and not string.find(text,"/::pos")then unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;local start=0;local d2=string.find(text,"/",start)-1;pos1=string.sub(text,start,d2)system.print(pos1)start=d2+2;d2=string.find(text,"/",start)-1;pos1time=tonumber(string.sub(text,start,d2))system.print(pos1time)start=d2+2;d2=string.find(text,"/",start)-1;pos2=string.sub(text,start,d2)system.print(pos2)start=d2+2;d2=string.find(text,"/",start)pos2time=tonumber(string.sub(text,start,d2))system.print(pos2time)system.print("---------------")system.print("The coordinates have been loaded successfully!")databank_1.setStringValue(1,pos1)databank_1.setFloatValue(2,pos1time)databank_1.setStringValue(3,pos2)databank_1.setFloatValue(4,pos2time)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;oldTime=tonumber(string.sub(text,start,d2))curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local d0=pos11:dist(pos22)local d1=pos2time-pos1time;tspeed=d0/d1;tspeed1=math.floor(d0/d1*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew "..lengthSU1 .." su, speed "..tspeed1 .." km/h")system.setWaypoint(Waypoint1)unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if GHUD_export_mode==true and string.find(text,"/::pos")then unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;local start=0;local d2=string.find(text,"/",start)-1;pos1=string.sub(text,start,d2)system.print(pos1)start=d2+2;d2=string.find(text,"/",start)-1;pos1time=tonumber(string.sub(text,start,d2))system.print(pos1time)start=d2+2;d2=string.find(text,"/",start)-1;pos2=string.sub(text,start,d2)system.print(pos2)start=d2+2;d2=string.find(text,"/",start)pos2time=tonumber(string.sub(text,start,d2))system.print(pos2time)system.print("---------------")system.print("The coordinates have been loaded successfully!")databank_1.setStringValue(1,pos1)databank_1.setFloatValue(2,pos1time)databank_1.setStringValue(3,pos2)databank_1.setFloatValue(4,pos2time)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;oldTime=tonumber(string.sub(text,start,d2))curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local d0=pos11:dist(pos22)local d1=pos2time-pos1time;tspeed=d0/d1;tspeed1=math.floor(d0/d1*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew "..lengthSU1 .." su, speed "..tspeed1 .." km/h")system.setWaypoint(Waypoint1)unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if string.find(text,"mar")then if showMarker==true then showMarker=false;system.print("Current target position - OFF")end;local d3=tonumber(text:sub(4))if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then local cU=d3*200000;local cV=databank_1.getStringValue(1)local cW=databank_1.getStringValue(3)pos111=zeroConvertToWorldCoordinates(cV,system)pos222=zeroConvertToWorldCoordinates(cW,system)local d4=vectorLengthen(pos111,pos222,cU)local d5=getPos4Vector(d4)system.print(d5 .." waypoint "..d3 .." su")end end end;function tickVector(unit,system,text)if targetTracker==true and targetVector.x~=0 and targetVector.y~=0 and targetVector.z~=0 then local d6=getPipeD(system)local d7=vec3(construct.getWorldOrientationUp()):normalize()local d8=vec3(construct.getWorldOrientationRight()):normalize()local d9=vec3(construct.getWorldOrientationForward()):normalize()local da=vec3(construct.getWorldVelocity()):normalize()local db=da:project_on_plane(d7)local dc=da:project_on_plane(d8)local dd=da:project_on_plane(d9)local de=dd:cross(d7):normalize()myAngleR=db:angle_between(d9)local df=utils.sign(de:angle_between(d9)-math.pi/2)if df~=0 then myAngleR=myAngleR*df;privMySignAngleR=df else myAngleR=myAngleR*privMySignAngleR end;local dg=dc:cross(d7):normalize()myAngleUp=dc:angle_between(-d7)-math.pi/2;local dh=utils.sign(dg:angle_between(d8)-math.pi/2)if dh~=0 then myAngleUp=myAngleUp*dh;privMySignAngleUp=dh else myAngleUp=myAngleUp*privMySignAngleUp end;local di=targetVector:normalize()local dj=di:project_on_plane(d7)local dk=di:project_on_plane(d8)local dl=di:project_on_plane(d9)local dm=dl:cross(d7):normalize()targetAngleR=dj:angle_between(d9)local dn=utils.sign(dm:angle_between(d9)-math.pi/2)if dn~=0 then targetAngleR=targetAngleR*dn;privTargetSignAngleR=dn else targetAngleR=targetAngleR*privTargetSignAngleR end;local dp=dk:cross(d7):normalize()targetAngleUp=dk:angle_between(-d7)-math.pi/2;local dq=utils.sign(dp:angle_between(d8)-math.pi/2)if dq~=0 then targetAngleUp=targetAngleUp*dq;privTargetSignAngleUp=dq else targetAngleUp=targetAngleUp*privTargetSignAngleUp end;targetVectorWidget=[[
         ...
         <br>
         <br>
         <bdr>]]..dH..[[</bdr> + <bdr>]]..dL..[[</bdr> : switch between friends/enemies<br>
         <br>
         <bdr>]]..dH..[[</bdr> + <bdr>]]..dK..[[</bdr> : construct size filter<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dw..[[</bdr> : add/remove selected target from whitelist<br>
         </div>
         <div class="helper2">
         <ibold>TARGET VECTOR:</ibold>
         <br>
         <br>
         <bdr>]]..dG..[[</bdr> : set pos1/pos2 for radar selected target<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>↓↑</bdr> : move destination ±2.5 su<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>←→</bdr> : move destination ±10 su<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dH..[[</bdr> : destination to closest target pipe<br>
         <br>
         <bdr>]]..dH..[[</bdr> + <bdr>]]..dG..[[</bdr> : on/off export mode<br>
         <br>
         <bdr>]]..dT..[[</bdr> : show/hide current target position (works only when manually setting coordinates or in export mode)<br>
         <br>
         <bdr>]]..dG..[[</bdr> + <bdr>]]..dz..[[</bdr> : switch target position between current speed or targetSpeed from LUA parameters<br>
         </div>
         </div>
         <div class="bottomL">
         <div class="helper3">
         ...
         <br>
         <br>
         <bdr>]]..dE..[[</bdr> : start/stop venting<br>
         <br>
         <bdr>]]..dD..[[</bdr> : on/off shield<br>
         <br>
         <bdr>]]..dC..[[</bdr> : switch AUTO/MANUAL shield mode<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dC..[[</bdr> : switch shield mode between MAX and 50/50 mode<br>
         <br>
         <bdr>]]..dB..[[</bdr> : agree and apply resists in manual shield mode<br>
         <br>
         <bdr>]]..dK..[[</bdr> + <bdr>]]..dw..[[</bdr> : 100% antimatter power<br>
         <br>
         <bdr>]]..dK..[[</bdr> + <bdr>]]..dx..[[</bdr> : 100% electromagnetic power<br>
         <br>
         <bdr>]]..dK..[[</bdr> + <bdr>]]..dy..[[</bdr> : 100% thermic power<br>
         <br>
         <bdr>]]..dK..[[</bdr> + <bdr>]]..dz..[[</bdr> : 100% kinetic power<br>
         <br>
         <bdr>]]..dL..[[</bdr> + <bdr>]]..dw..[[</bdr> : cannon profile<br>
         <br>
         <bdr>]]..dL..[[</bdr> + <bdr>]]..dx..[[</bdr> : railgun profile<br>
         <br>
         <bdr>]]..dL..[[</bdr> + <bdr>]]..dy..[[</bdr> : laser profile<br>
         <br>
         <bdr>]]..dL..[[</bdr> + <bdr>]]..dz..[[</bdr> : universal profile<br>
         </div>
         <div class="helper2">
         <ibold>Other:</ibold>
         <br>
         <br>
         <bdr>]]..dR..[[</bdr> + <bdr>]]..dF..[[</bdr> : lock brake<br>
         <br>
         <bdr>]]..dw..[[</bdr> : show/hide planets and planetary periscope<br>
         <br>
         <bdr>]]..dx..[[</bdr> : set destination to planet #1 (closest pipe planets)<br>
         <br>
         <bdr>]]..dy..[[</bdr> : set destination to closest pipe<br>
         <br>
         <bdr>]]..dz..[[</bdr> : set destination to planet #2 (closest pipe planets)<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dx..[[</bdr> : set destination to destination planet (LUA parameters)<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dy..[[</bdr> : set destination to custom pipe Destination - Departure (LUA parameters)<br>
         <br>
         <bdr>]]..dF..[[</bdr> + <bdr>]]..dz..[[</bdr> : set destination to departure planet (LUA parameters)<br>
         <br>
         <bdr>]]..dA..[[</bdr> : Helios system map<br>
         </div>
         </div>
         <div class="bottomL">
         <div class="helper3">
