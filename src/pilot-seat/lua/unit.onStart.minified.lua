...
<text x="%f" y="%f" font-size="11px" fill="yellow">%s</text>
      ...
      .warp-scan {
         ...
         background: #ff3a56;
                        ...
                        </div>]]end end end;local cM=0;local cN=0;local cO=0;if activeRadar.isConstructIdentified(aW)==1 and size~=""then local cF=activeRadar.getConstructName(aW)local cH=math.floor(activeRadar.getConstructDistance(aW))if cH>=1000 then cH=''..string.format('%0.1f',cH/1000)..' km ('..string.format('%0.2f',cH/200000)..' su)'else cH=''..cH..' m'end;local cP=tostring(aW):sub(-3)local cQ=''..cP..' '..cF..''isILock=true;cM=math.floor(activeRadar.getConstructSpeed(aW)*3.6)if activeRadar.getTargetId(aW)==aW then cu=cu..[[
                     ...
                     </div>]]end end;if G>50 then G=0;coroutine.yield()end end;if GHUD_show_allies==true then if co>0 then cv="<alliescolor>Allies:</alliescolor><br><countcolor>"..co.."</countcolor> <countcolor2>"..conID.."</countcolor2>"else cv="<alliescolor>Allies:</alliescolor><br><countcolor>0</countcolor> <countcolor2>"..conID.."</countcolor2>"end;cl=htmlbasic..[[
               <style>
               .th3>.table-cell3 {
            ...
            </div>]]if GHUD_show_echoes==true then local cT=''for cD,aW in pairs(radarDynamicData)do cT=cT..'<span style="left:'..cD..'px;height:'..aW..'px;"></span>'end;local cU=''for cD,aW in pairs(radarStaticData)do cU=cU..'<span style="left:'..cD..'px;height:'..aW..'px;"></span>'end;local htmlRadar=htmlRadar..[[
