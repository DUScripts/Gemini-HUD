radarIDs=activeRadar.getConstructIds()idN=#radarIDs;mRadar:onUpdate()if coroutine.status(main1)~="dead"and coroutine.status(main1)=="suspended"then coroutine.resume(main1)end;if coroutine.status(main2)~="dead"and coroutine.status(main2)=="suspended"then coroutine.resume(main2)end;if corpos==true then if coroutine.status(ck)~="dead"and coroutine.status(ck)=="suspended"then coroutine.resume(ck,asteroidcoord)end end;damage_SVG()varcombat=construct.getPvPTimer()if varcombat>302 then local a=shield.getStressRatioRaw()AM_stress=a[1]EM_stress=a[2]KI_stress=a[3]TH_stress=a[4]end;ccs_SVG()local b=shield.getShieldHitpoints()/shieldMaxHP*100;local c=string.format('%0.0f',math.ceil(b))if shield.isActive()==0 then svghp=maxSHP*b*0.01;shieldColor="#fc033d"shieldStatus="DEACTIVE"else shieldColor="#2ebac9"shieldStatus="ACTIVE"end;if system.getArkTime()-lastShotTime>=40 then shoteCount=0 end;resisttime=shield.getResistancesCooldown()if resisttime~=0 then resCLWN=math.floor(resisttime)else resCLWN=''end;if shield.isVenting()==1 then shieldStatus='VENTING'end;venttime=shield.getVentingCooldown()if venttime~=0 then resCLWN=math.floor(venttime)end;if mybr==true then brakeHUD=[[
   <style>
   .main1 {
      position: absolute;
      width: content;
      ...
      <div class="main1">BRAKE ENGAGED</div>]]else brakeHUD=''end;local d=''if math.ceil(b)<=50 then d=[[<style>
      .warningmsg {
         position: absolute;
         ...
         <div class="warningmsg">SHIELD LOW</div>]]end;if t2==true then blink=blink+0.015;if blink>=1 then t2=false end end;if t2==false then blink=blink-0.015;if blink<0.4 then t2=true end end;if math.ceil(b)<=35 then shieldAlarm=true;if alarmTimer==true then d=[[<style>
            .warningmsg {
               position: absolute;
               top: ]]..GHUD_shield_warning_message_Y..[[%;
               left: 50%;
               ...
               <div class="warningmsg">SHIELD LOW</div>]]end else shieldAlarm=false end;local e=vec3(construct.getWorldPosition())local f=math.floor(unit.getThrottle())local g=math.floor(json.decode(unit.getWidgetData()).acceleration/9.80665*10)/10;local h=construct.getWorldVelocity()local i=math.floor(vec3(h):len()*3.6)local j=math.floor(construct.getMaxSpeed()*3.6)local k=''local l=''local m=''local n=''local o=''local p=e+400000*vec3(h)local q=library.getPointOnScreen({p.x,p.y,p.z})if q[3]>0 and i>15 then local r=screenWidth*q[1]-GHUD_flight_indicator_size/2;local s=screenHeight*q[2]-GHUD_flight_indicator_size/2;o=[[
            <style>
            .flightIndicator {
               position: absolute;
            ...
            <line style="fill: ]]..GHUD_flight_indicator_color..[[; stroke: ]]..GHUD_flight_indicator_color..[[; stroke-width: 20px;" x1="10" y1="100" x2="190" y2="100" transform="matrix(0.707107, 0.707107, -0.707107, 0.707107, 100, -41.421356)"></line>
            </svg></div>]]end;local t,u,v,w=safeZone()if szsafe==true then safetext=''..t..' <green1>'..v..' '..w..'</green1>'local x=library.getPointOnScreen({u.x,u.y,u.z})if x[3]>0 then local y=screenWidth*x[1]-50;local z=screenHeight*x[2]-50;m=[[
               <style>
               .pvpzoneAR {
                  position: absolute;
                  width: 100px;
               ...
               <text style="fill: white; font-family: verdana; font-size: 28px; font-weight: 700; text-anchor: middle;" x="125" y="209.955">]]..v..[[</text>
               <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 28px; font-style: italic; font-weight: 700; text-anchor: middle;" x="125" y="240.424">]]..w..[[</text>
               </svg></div>]]end else safetext=''..t..' <green1>'..v..' '..w..'</green1>'local x=library.getPointOnScreen({u.x,u.y,u.z})if x[3]>0 then local y=screenWidth*x[1]-50;local z=screenHeight*x[2]-50;n=[[
               <style>
               .safezoneAR {
                  position: absolute;
                  width: 100px;
               ...
               <text style="fill: white; font-family: verdana; font-size: 28px; font-weight: 700; text-anchor: middle;" x="125" y="209.955">]]..v..[[</text>
               <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 28px; font-style: italic; font-weight: 700; text-anchor: middle;" x="125" y="240.424">]]..w..[[</text>
               </svg></div>]]end end;if asteroidcoord[1]~=0 then local x=library.getPointOnScreen({asteroidcoord.x,asteroidcoord.y,asteroidcoord.z})if x[3]>0 then local A=vec3(e-asteroidcoord):len()local B=''if A>=100000 then A=string.format('%0.2f',A/200000)B='SU'elseif A>=1000 and A<100000 then A=string.format('%0.1f',A/1000)B='KM'else A=string.format('%0.0f',A)B='M'end;local r=screenWidth*x[1]-50;local s=screenHeight*x[2]-50;l=[[
               <style>
               ...
               </svg></div>]]end end;if DisplayRadar==true then local r,s,C=table.unpack(construct.getWorldOrientationForward())local D=math.floor(math.atan(r,s)*180/math.pi+180)local E=math.floor(math.atan(s,C)*180/math.pi+180)local F=[[
            <style>
            .XY {
               position: absolute;
               left: 2%;
            ...
            <div class="XY">X: ]]..D..[[<br>Y: ]]..E..[[</div>]]message=[[
            <style>
            .svg {
               position:absolute;
               left: 0;
               ...
               .wptxt {
                  ...
                  .shiptxt {
                     ...
                     text-anchor: start;
                  ...
                  </style>]]message=message..[[<svg class="svg">]]svgradar=""RadarX=screenWidth*1/7;RadarY=screenWidth*1/7;RadarR=screenWidth*1/7;svgradar=svgradar..string.format([[<line x1="%f" y1="%f" x2="%f" y2="%f" stroke-width="2" stroke="black" />]],RadarX,RadarY-RadarR,RadarX,RadarY+RadarR)svgradar=svgradar..string.format([[<line x1="%f" y1="%f" x2="%f" y2="%f" stroke-width="2" stroke="black" />]],RadarX-RadarR,RadarY,RadarX+RadarR,RadarY)svgradar=svgradar..string.format([[<circle  cx="%f" cy="%f" r="%f" stroke="black" fill="transparent" stroke-width="5"/>]],RadarX,RadarY,RadarR/2)svgradar=svgradar..string.format([[<circle  cx="%f" cy="%f" r="%f" stroke="black" fill-opacity="0.2" fill="green" stroke-width="5"/>]],RadarX,RadarY,RadarR)for G in pairs(atlas[0])do local H=atlas[0][G]if H.type[1]=='Planet'or H.isSanctuary==true then drawonradar(vec3(H.center),H.name[1])local x=library.getPointOnScreen({H.center[1],H.center[2],H.center[3]})if x[3]>0 then local A=vec3(e-vec3(H.center)):len()local B=''if A>=100000 then A=string.format('%0.2f',A/200000)B='SU'elseif A>=1000 and A<100000 then A=string.format('%0.1f',A/1000)B='KM'else A=string.format('%0.0f',A)B='M'end;local y=screenWidth*x[1]-50;local z=screenHeight*x[2]-50;k=k..[[
                           <style>
                           .pl]]..H.name[1]..[[ {
                              position: absolute;
                              width: 100px;
                           ...
                           <div class="pl]]..H.name[1]..[["><?xml version="1.0" encoding="utf-8"?>
                           <svg viewBox="0 0 250 250" xmlns="http://www.w3.org/2000/svg">
                           <ellipse style="fill: rgba(0, 0, 0, 0); stroke: #FFB12C; stroke-width: 8px;" cx="125" cy="125" rx="50" ry="50"/>
                           <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 28px; font-style: italic; font-weight: 700; text-anchor: middle;" x="125" y="48.955">]]..H.name[1]..[[</text>
                           <text style="fill: white; font-family: verdana; font-size: 28px; font-weight: 700; text-anchor: middle;" x="125" y="209.955">]]..A..[[</text>
                           <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 28px; font-style: italic; font-weight: 700; text-anchor: middle;" x="125" y="240.424">]]..B..[[</text>
                           </svg></div>]]end end end;drawonradar(u,t)if asteroidcoord[1]~=0 then drawonradar(asteroidcoord,""..GHUD_marker_name.."")end;message=message..svgradar..F;message=message.."</svg>"else message=''end;local I=''if weapon_1~=nil then local J=weapon_1.getWidgetData()probil=tonumber(J:match('"hitProbability":(.-),'))end;local K=vec3(construct.getWorldPosition())local L=activeRadar.getTargetId()if L~=0 then local B=""local A=math.floor(activeRadar.getConstructDistance(L))local M=activeRadar.getConstructName(L)local N=activeRadar.getConstructCoreSize(L)local i='UNKNOWN'local O='middle'local P='0.0'if totalDamage[L]~=nil then P=string.format('%0.1f',totalDamage[L].damage*0.000001)end;if activeRadar.isConstructIdentified(L)==1 then i=activeRadar.getConstructSpeed(L)i=math.floor(i*3.6)dist3=i;O='start'end;local Q=K+A*vec3(construct.getWorldOrientationForward())local x=library.getPointOnScreen({Q.x,Q.y,Q.z})if A>=100000 then A=string.format('%0.2f',A/200000)B='SU'elseif A>=1000 and A<100000 then A=string.format('%0.1f',A/1000)B='KM'else A=string.format('%0.0f',A)B='M'end;local R=[[
                  .sight1 {
                     position: absolute;
                     opacity: 0;
                     left: 0;
                  ...
                  ]]if x[3]>0 then local y=screenWidth*x[1]-GHUD_AR_sight_size/2;local z=screenHeight*x[2]-GHUD_AR_sight_size/2;R=[[
                     .sight1 {
                        position: absolute;
                  ...
                  <text style="fill: white; font-family: verdana; font-size: 26px; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: middle;" transform="matrix(1, 0, 0, 1, 45.470986, 456.61146)"><tspan x="254.529" y="60.003">]]..P..[[M</tspan></text>
                  <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 26px; font-style: italic; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: middle;" transform="matrix(1, 0, 0, 1, 44.105107, 38.795308)"><tspan x="254.529" y="36.003">]]..M..[[</tspan></text>
                  ...
                  <text style="fill: rgb(0, 191, 255); font-family: verdana; font-size: 26px; font-style: italic; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: end;" transform="matrix(1, 0, 0, 1, -154.09122, 244.195302)"><tspan x="254.529" y="36.003">]]..B..[[</tspan></text>
                  ...
                  <text style="fill: white; font-family: verdana; font-size: 26px; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: middle;" transform="matrix(1, 0, 0, 1, 45.470986, 44.611463)"><tspan x="254.529" y="60.003">]]..tostring(L):sub(-3)..[[</tspan></text>
                  <text style="fill: white; font-family: verdana; font-size: 26px; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: end;" y="310.246" x="101.677">]]..A..[[</text>
                  <text style="fill: ]]..newcolor..[[; font-family: verdana; font-size: 26px; font-weight: 700; paint-order: stroke; stroke: rgb(0, 0, 0); stroke-width: 2px; text-anchor: ]]..O..[[;" y="310.246" x="494">]]..i..znak..[[</text>
                  </svg></div>]]else local Q=K+400000*vec3(construct.getWorldOrientationForward())local x=library.getPointOnScreen({Q.x,Q.y,Q.z})if x[3]>0 then local y=screenWidth*x[1]-GHUD_AR_sight_size/2;local z=screenHeight*x[2]-GHUD_AR_sight_size/2;I=[[
                     <style>
                     .sight1 {
                        position: absolute;
                        width: ]]..GHUD_AR_sight_size..[[px;
                     ...
                     <svg viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xmlns="http://www.w3.org/2000/svg">
                     <path style="fill: ]]..GHUD_AR_sight_color..[[;" d="M 231.231 440.732 C 230.524 444.711 227.058 447.508 223.154 447.508 C 222.675 447.508 222.191 447.465 221.705 447.379 C 141.949 433.179 78.822 370.052 64.621 290.296 C 63.825 285.83 66.802 281.564 71.268 280.769 C 75.729 279.975 80 282.95 80.794 287.416 C 93.793 360.422 151.578 418.207 224.583 431.205 C 229.051 432.001 232.026 436.266 231.231 440.732 Z M 71.268 231.231 C 71.754 231.318 72.239 231.36 72.717 231.36 C 76.621 231.36 80.087 228.563 80.794 224.584 C 93.793 151.578 151.578 93.793 224.583 80.795 C 229.049 80 232.026 75.734 231.23 71.269 C 230.435 66.802 226.166 63.829 221.704 64.622 C 141.948 78.822 78.821 141.949 64.62 221.705 C 63.825 226.171 66.802 230.436 71.268 231.231 Z M 440.732 280.769 C 436.273 279.976 432.001 282.951 431.206 287.416 C 418.207 360.422 360.422 418.208 287.417 431.206 C 282.951 432.001 279.974 436.267 280.77 440.733 C 281.477 444.712 284.942 447.509 288.847 447.509 C 289.326 447.509 289.81 447.466 290.296 447.38 C 370.052 433.18 433.179 370.052 447.38 290.296 C 448.175 285.83 445.198 281.564 440.732 280.769 Z M 387.492 112.892 L 399.109 124.508 C 423.609 151.152 440.78 184.629 447.38 221.703 C 448.176 226.169 445.199 230.435 440.733 231.23 C 436.27 232.026 432.001 229.049 431.207 224.583 C 425.206 190.887 409.664 160.43 387.49 136.126 L 375.873 124.51 C 351.57 102.336 321.113 86.793 287.417 80.793 C 282.951 79.998 279.974 75.732 280.77 71.266 C 281.565 66.8 285.839 63.825 290.296 64.619 C 327.37 71.22 360.848 88.39 387.492 112.892 Z" transform="matrix(0.707107, 0.707107, -0.707107, 0.707107, 255.999945, -106.038815)"></path>
                     </svg></div>]]end end;local S=''if GHUD_show_AR_allies_marks==true then for T,U in pairs(radarIDs)do if activeRadar.hasMatchingTransponder(U)==1 then local V=activeRadar.getConstructWorldPos(U)local W=library.getPointOnScreen({V[1],V[2],V[3]})if W[3]>0 then local r=screenWidth*W[1]-GHUD_AR_allies_border_size/2;local s=screenHeight*W[2]-GHUD_AR_allies_border_size/2;S=S..[[
                           <style>
                           .id]]..U..[[ {
                              position: absolute;
                           ...
                           <div class="id]]..U..[["><?xml version="1.0" encoding="utf-8"?>
                           <svg viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">
                           <rect x="235" y="235" width="30" height="30" style="fill: rgba(0,0,0,0); stroke: ]]..GHUD_AR_allies_border_color..[[; stroke-width: 2"/>
                           <text style="fill: ]]..GHUD_AR_allies_font_color..[[; font-family: Arial, sans-serif; font-size: 28px; font-weight: 700; text-anchor: middle;" transform="matrix(0.609174, 0, 0, 0.609176, 250.000005, 227)">]]..tostring(U):sub(-3)..[[</text>
                           </svg></div>]]end end end end;local X=''local Y=''local Z=''if GHUD_show_hits==true then for T,U in pairs(lastHitTime)do if lastHitTime[T]~=nil then lastHitTime[T].time=lastHitTime[T].time+0.025;lastHitTime[T].hitOpacity=lastHitTime[T].hitOpacity-0.015;local _=GHUD_hits_misses_Y-lastHitTime[T].time*3.25;local a0=GHUD_hit_X+lastHitTime[T].time*2;if lastHitTime[T].hitOpacity<=0 then lastHitTime[T].hitOpacity=0 end;local a1=[[
                        <style>
                        .hit]]..T..[[ {
                           top: ]].._..[[vh;
                           left: ]]..a0 ..[[%;
                           ...
                           font-weight: bold;
                           color: #FFB12C;
                           opacity: ]]..lastHitTime[T].hitOpacity..[[;
                           transform: translate(-50%, -50%);
                        }
                        </style>
                        <div class="hit]]..T..[[">HIT ]]..lastHitTime[T].damage..[[ HP</div>]]hits[T]={html=a1}if lastHitTime[T].time>=2 then hits[T]={html=''}if lastHitTime[T].anims==hitAnimations then hits[T]=nil;hits={}hitAnimations=0;lastHitTime={}end end end end end;if GHUD_show_misses==true then for T,U in pairs(lastMissTime)do if lastMissTime[T]~=nil then lastMissTime[T].time=lastMissTime[T].time+0.025;lastMissTime[T].missOpacity=lastMissTime[T].missOpacity-0.015;local _=GHUD_hits_misses_Y-lastMissTime[T].time*3.25;local a2=GHUD_miss_X-lastMissTime[T].time*2;if lastMissTime[T].missOpacity<=0 then lastMissTime[T].missOpacity=0 end;local a3=[[
                        <style>
                        .miss]]..T..[[ {
                           top: ]].._..[[vh;
                           left: ]]..a2 ..[[%;
                           ...
                           font-weight: bold;
                           color: #fc033d;
                           opacity: ]]..lastMissTime[T].missOpacity..[[;
                           transform: translate(-50%, -50%);
                        }
                        </style>
                        <div class="miss]]..T..[[">MISS</div>]]misses[T]={html=a3}if lastMissTime[T].time>=2 then misses[T]={html=''}if lastMissTime[T].anims==missAnimations then misses[T]=nil;misses={}missAnimations=0;lastMissTime={}end end end end end;if GHUD_show_hits==true then for T,U in pairs(hits)do if hits[T]~=nil then X=X..hits[T].html end end end;if GHUD_show_misses==true then for T,U in pairs(misses)do if misses[T]~=nil then Y=Y..misses[T].html end end end;for T,U in pairs(target)do if target[T]~=nil then if target[T].left>85 and target[T].one==true then target[T].left=target[T].left-0.3 end;if target[T].left<=85 then target[T].left=85;target[T].one=false end;local a4=[[
                     <style>
                     .targ]]..T..[[ {
                        position: relative;
                     ...
                     <div class="targ]]..T..[[">[]]..target[T].size1 ..[[] ]]..target[T].id..[[ ]]..target[T].name1 ..[[</div>]]targets[T]={html=a4}if target[T].one==false then target[T].delay=target[T].delay+1;if target[T].delay>=100 then target[T].opacity=target[T].opacity-0.01;if target[T].opacity<=0 and target[T].cnt==count then target[T]=nil;target={}targets={}count=0 end end end end end;for T,U in pairs(targets)do if targets[T]~=nil then Z=Z..targets[T].html end end;local a5=[[
               <html>
               <style>
               html,body {
                  margin:0;
               ...
               <div class="right1">THRUST<br><div class="thrust1">]]..f..[[</div><orange1>%</orange1><br>SPEED<br><div class="speed1">]]..i..[[</div><orange1>KM/H</orange1><mspeed> ]]..j..[[</mspeed><br>ACCEL<br><div class="accel1">]]..g..[[</div><orange1>G</orange1><br>BRAKE-DISTANCE<br><div class="brakedist">]]..brakeDist..[[</div><orange1>]]..brakeS..[[</orange1></div>
               ...
               </svg></div><br><div class="shieldtext">]]..c..[[</div><orange1>%</orange1><br>FUEL<div class="fuel1"><svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
               ...
               <text style="fill: ]]..GHUD_shield_text_color..[[; font-family: Arial, sans-serif; font-size: 20px; font-weight: 700; paint-order: stroke; stroke: ]]..GHUD_shield_text_stroke_color..[[; stroke-width: 1.25px; text-anchor: middle;" x="252" y="257.079">]]..c..[[%</text>
               ]]..AM_res..[[
               ]]..EM_res..[[
               ]]..KI_res..[[
               ]]..TH_res..[[
               ...
               </html>]]if map==0 and helper==false and helper1==false then system.setScreen(a5)end