...
<text x="%f" y="%f" font-size="11px" fill="yellow">%s</text>
      ...
      .warp-scan {
         ...
         background: #ff3a56;
      ...
      </style>]]Kinematic={}function Kinematic.computeAccelerationTime(b7,b8,b9)return(b9-b7)/b8 end;function Kinematic.computeDistanceAndTime(b7,b9,ba,bb,bc,bd)bc=bc or 0;bd=bd or 0;local be=b7<b9;local bf=bb/(be and ba or-ba)local bg=-bd/ba;local bh=bf+bg;if b7==b9 then return 0,0 elseif be and bh<=0 or not be and bh>=0 then return-1,-1 end;local bi,bj=0,0;if bf~=0 and bc>0 then local bk=math.pi/bc/2;local aW=function(bl)return bf*(bl/2-bc*math.sin(bk*bl)/math.pi)+bg*bl+b7 end;local bm=be and function(bn)return bn>=b9 end or function(bn)return bn<=b9 end;bj=2*bc;if bm(aW(bj))then local lasttime=0;while math.abs(bj-lasttime)>0.25 do local bl=(bj+lasttime)/2;if bm(aW(bl))then bj=bl else lasttime=bl end end end;local bo=2*bf*bc^2/math.pi^2;bi=bo*(math.cos(bk*bj)-1)+(bf+2*bg)*bj^2/4+b7*bj;if bj<2*bc then return bi,bj end;b7=aW(bj)end;local ag=bf+bg;local bl=Kinematic.computeAccelerationTime(b7,ag,b9)local bp=b7*bl+ag*bl*bl/2;return bi+bp end;if GHUD_radar_notifications_border_radius==true then GHUD_border_radius='15px'else GHUD_border_radius='none'end;GHUD_allies_count1=GHUD_allies_count+1;if GHUD_windowed_mode then GHUD_allies_Y=2 end;GHUD_show_echoes=false;if GHUD_show_echoes==true then statusY=13.5 else statusY=6 end;atlas=require("atlas")activeRadar=radar_1;activeRadar.setSortMethod(1)shift=false;lalt=false;radarIDs={}idN=0;GHUD_show_allies=true;screenHeight=system.getScreenHeight()screenWidth=system.getScreenWidth()startTime=system.getArkTime()lastHitTime={}lastMissTime={}hits={}misses={}hitAnimations=0;missAnimations=0;totalDamage={}mRadar={}mWeapons={}size={'XL','L','M','S','XS','ALL'}defaultSize='ALL'sizeState=6;focus=''gunnerHUD=''vectorHUD=''downB=false;upB=false;atmovar=false;endload=0;znak=''newcolor="white"dist1=0;dist3=0;probil=0;playerName=system.getPlayerName(player.getId())warpScan=0;t_radarEnter={}loglist={}radarTarget=nil;newWhitelist={}radarStatic={}radarDynamic={}radarStaticWidget={}radarStaticData={}radarDynamicWidget={}radarDynamicData={}radarWidget=''targets={}target={}count=0;gearB=false;helper=false;helper1=false;pp1=''shipName=construct.getName()local bq=construct.getId()system.print(''..shipName..': '..bq..'')conID=tostring(bq):sub(-3)GHUD_friendly_IDs={}local br=databank_2.getNbKeys()if br>0 then for G=1,br do table.insert(GHUD_friendly_IDs,databank_2.getIntValue(G))end;system.print('Databank whitelist loaded')end;function checkWhitelist()local whitelist=GHUD_friendly_IDs;local bs={}for bt,bu in ipairs(whitelist)do bs[bu]=true end;return bs end;function table.contains(table,bv)for bt,bw in pairs(table)do if bw==bv then return true end end;return false end;whitelist=checkWhitelist()local bx=100;radarWidgetScale=2;radarWidgetScaleDisplay='<div class="measures"><span>0 SU</span><span>1 SU</span><span>2 SU</span></div>'function defaultRadar()sizeState=6;defaultSize='ALL'if mRadar.friendlyMode==true then mRadar.friendlyMode=false end end;function mRadar:createWidget()self.dataID=self.system.createData(activeRadar.getWidgetData())radarPanel=self.system.createWidgetPanel('')radarWidget=self.system.createWidget(radarPanel,activeRadar.getWidgetType())self.system.addDataToWidget(self.dataID,radarWidget)end;function mRadar:createWidgetNew()self.dataID=self.system.createData(activeRadar.getWidgetData())radarWidget=self.system.createWidget(radarPanel,activeRadar.getWidgetType())self.system.addDataToWidget(self.dataID,radarWidget)end;function mRadar:deleteWidget()self.system.destroyData(self.dataID)self.system.destroyWidget(radarWidget)end;function mRadar:updateLoop()while true do self:updateStep()coroutine.yield()end end;function mRadar:updateStep()local by={}local bz=activeRadar.getWidgetData()local bA=bz:gmatch('({"constructId":".-%b{}.-})')local bB=next(self.idFilter)~=nil;local G=0;for bC in bA do G=G+1;if G%bx==0 then coroutine.yield()end;local bD=tonumber(bC:match('"constructId":"([%d]*)"'))local size=activeRadar.getConstructCoreSize(bD)local bE=activeRadar.isConstructIdentified(bD)local bF=activeRadar.isConstructAbandoned(bD)local bG=activeRadar.getTargetId(bD)if bE==1 or bF==0 or bG==bD and size~=""then if defaultSize=='ALL'then if(whitelist[bD]==true or activeRadar.hasMatchingTransponder(bD)==1)~=self.friendlyMode and activeRadar.getThreatRateFrom(bD)~=5 then goto bH end;if bB and self.idFilter[bD%1000]~=true then goto bH end;by[#by+1]=bC:gsub('"name":"(.+)"','"name":"'..tostring(bD):sub(-3)..' - %1"')::bH::elseif size==defaultSize then if(whitelist[bD]==true or activeRadar.hasMatchingTransponder(bD)==1)~=self.friendlyMode and activeRadar.getThreatRateFrom(bD)~=5 then goto bI end;if bB and self.idFilter[bD%1000]~=true then goto bI end;by[#by+1]=bC:gsub('"name":"(.+)"','"name":"'..tostring(bD):sub(-3)..' - %1"')::bI::end end end;local bJ=(bB and''..focus..' - FOCUS - 'or'')..(self.friendlyMode and''..defaultSize..' - Friends'or''..defaultSize..' - Enemies')local bK=bz:match('"currentTargetId":".+')bK=bK:gsub('"errorMessage":""','"errorMessage":"'..bJ..'"')bz='{"constructsList":['..table.concat(by,",").."],"..bK;self.system.updateData(self.dataID,bz)end;function mRadar:onUpdate()coroutine.resume(self.updaterCoroutine)end;function mRadar:clearIDFilter()self.idFilter={}end;function mRadar:addIDFilter(bL)self.idFilter[bL]=true end;function mRadar:onTextInput(text)self:clearIDFilter()focus=text:sub(-3)defaultRadar()if focus=='f'then system.print('Focus mode deactivated')else system.print('Focus ID: '..focus)end;for bL in text:gmatch('%D(%d%d%d)')do self:addIDFilter(tonumber(bL))end end;function mRadar:toggleFriendlyMode()self.friendlyMode=not self.friendlyMode end;function mRadar:new(bM)local mRadar={}setmetatable(mRadar,self)self.system=bM;self.friendlyMode=false;self.onlyIdentified=false;self.idFilter={}self:createWidget()self.updaterCoroutine=coroutine.create(function()self:updateLoop()end)return self end;local bN={}local bO={}local bP={}local bQ={}local bR={}local bS={}function mWeapons:createWidgets()if not(type(self.weapons)=='table'and#self.weapons>0)then return end;local bT;for G,bU in ipairs(self.weapons)do if(G-1)%self.weaponsPerPanel==0 then bT=self.system.createWidgetPanel('')end;local bV=self.system.createData(bU.getWidgetData())self.weaponData[bV]=bU;bN[bV]=0;self.system.addDataToWidget(bV,self.system.createWidget(bT,bU.getWidgetType()))end end;function mWeapons:onUpdate()for bV,bU in pairs(self.weaponData)do local bW=bU.getWidgetData()local bX=bW:match('"weaponStatus":(%d+)')local bY=tonumber(bW:match('"cycleAnimationRemainingTime":(.-),'))local bZ=bW:match('"fireReady":(.-),')local b_=bW:match('"outOfZone":(.-),')local c0=bW:match('"constructId":"(.-)"')local c1=bW:match('"hitProbability":(.-),')local c2=math.floor(tonumber(c1)*100)local c3=bY>bN[bV]bN[bV]=bY;if bX==bO[bV]and bR[bV]==c0 and bP[bV]==bZ and bQ[bV]==b_ and bS[bV]==c1 and not c3 then goto c4 end;bO[bV]=bX;bP[bV]=bZ;bQ[bV]=b_;bR[bV]=c0;bS[bV]=c1;local c5=bW:match('"ammoName":"(.-)"')local c6=""if c5:match("Antimatter")then c6="AM"elseif c5:match("Electromagnetic")then c6="EM"elseif c5:match("Kinetic")then c6="KI"elseif c5:match("Thermic")then c6="TH"elseif c5:match("Stasis")then c6="Stasis"end;local c7=""if c5:match("Precision")then c7="Prec"elseif c5:match("Heavy")then c7="Heavy"elseif c5:match("Agile")then c7="Agile"elseif c5:match("Defense")then c7="Def"end;bW=bW:gsub('"constructId":"(%d+(%d%d%d))","name":"(.?.?.?.?).-"','"constructId":"%1","name":"%2 - %3"')bW=bW:gsub('"ammoName":"(.-)"','"ammoName":"'..c2 ..'%% - '..c6 ..' '..c7 ..'"')if self.system.updateData(bV,bW)~=1 then self.system.print('update error')end::c4::end end;function mWeapons:new(bM,c8,c9)local mWeapons={}setmetatable(mWeapons,self)self.system=bM;self.weapons=c8;self.weaponsPerPanel=c9 or 3;self.weaponData={}self:createWidgets()return self end;function coroutine.xpcall(q)local r={coroutine.resume(q)}if r[1]==false then local s=traceback(q)local t=s:gsub('"%-%- |STDERROR%-EVENTHANDLER[^"]*"','chunk')system.print(t)t=r[2]:gsub('"%-%- |STDERROR%-EVENTHANDLER[^"]*"','chunk')system.print(t)return false,r[2],s end;return table.unpack(r)end;function ConvertLocalToWorld(au,y,av)local ca=au*vec3(construct.getWorldRight())local cb=y*vec3(construct.getWorldForward())local cc=av*vec3(construct.getWorldUp())return ca+cb+cc+vec3(construct.getWorldPosition())end;if GHUD_radarWidget_on_top==true then mRadar=mRadar:new(system)if weapon_1~=nil then mWeapons=mWeapons:new(system,weapon,GHUD_weapon_panels)end else if weapon_1~=nil then mWeapons=mWeapons:new(system,weapon,GHUD_weapon_panels)end;mRadar=mRadar:new(system)end;function main()while true do local G=0;local cd=""local ce=""local cf=""local cg=0;local ch=0;local ci=0;local cj,ck,cl="","",""local cm=""local cn=""local co=""local cp=""local cq=""local cr=""local cs=""local ct=""local cu=""radarTarget={}radarStatic={}radarDynamic={}radarDynamicData=radarDynamicWidget;radarDynamicWidget={}radarStaticData=radarStaticWidget;radarStaticWidget={}if radar_2~=nil then if radar_1.getOperationalState()==-1 and atmovar==false then atmovar=true;activeRadar=radar_2;mRadar:deleteWidget()mRadar:createWidgetNew()radarWidgetScale=160;radarWidgetScaleDisplay='<div class="measures"><span>0 KM</span><span>2.5 KM</span><span>5 KM</span></div>'activeRadar.setSortMethod(1)end;if radar_1.getOperationalState()==1 and atmovar==true then atmovar=false;activeRadar=radar_1;mRadar:deleteWidget()mRadar:createWidgetNew()radarWidgetScale=2;radarWidgetScaleDisplay='<div class="measures"><span>0 SU</span><span>1 SU</span><span>2 SU</span></div>'activeRadar.setSortMethod(1)end end;for cv,aW in pairs(radarIDs)do G=G+1;local size=activeRadar.getConstructCoreSize(aW)local cw={}if t_radarEnter[aW]~=nil then if activeRadar.hasMatchingTransponder(aW)==0 and not whitelist[aW]and size~=""and activeRadar.getConstructDistance(aW)<600000 then local cx=activeRadar.getConstructName(aW)if activeRadar.isConstructAbandoned(aW)==0 then local cy='NEW TARGET: '..cx..' - '..aW..' - Size: '..size..'\n '..t_radarEnter[aW].pos..''table.insert(loglist,cy)if count<10 then count=count+1;if target[count]==nil then target[count]={left=100,opacity=1,cnt=count,name1=cx,size1=size,id=tostring(aW):sub(-3),one=true,check=true,delay=0}end;system.playSound('enter.mp3')end else local d=activeRadar.getConstructWorldPos(aW)d='::pos{0,0,'..d[1]..','..d[2]..','..d[3]..'}'local cy='NEW TARGET (abandoned): '..cx..' - '..aW..' - Size: '..size..'\n '..d..''table.insert(loglist,cy)if count<10 then count=count+1;if target[count]==nil then target[count]={left=100,opacity=1,cnt=count,name1=cx,size1=size,id=tostring(aW):sub(-3),one=true,check=true,delay=0}end end;system.playSound('sonar.mp3')end end;t_radarEnter[aW]=nil end;if GHUD_show_echoes==true then if size~=""then cw.widgetDist=math.ceil(activeRadar.getConstructDistance(aW)/1000*radarWidgetScale)end end;if GHUD_show_allies==true and size~=""then if activeRadar.hasMatchingTransponder(aW)==1 or whitelist[aW]and activeRadar.getThreatRateFrom(aW)~=5 then local cx=activeRadar.getConstructName(aW)local cz=math.floor(activeRadar.getConstructDistance(aW))if cz>=1000 then cz=''..string.format('%0.1f',cz/1000)..'km ('..string.format('%0.2f',cz/200000)..'SU)'else cz=''..cz..'m'end;local cA=tostring(aW):sub(-3)local cB=''..cA..' '..cx..''cg=cg+1;if activeRadar.getTargetId(aW)~=aW and cg<GHUD_allies_count1 then cj=cj..[[
                        <div class="table-row3 th3">
                        <div class="table-cell3">
                        ]]..'['..size..'] '..cB..[[<br><distalliescolor>]]..cz..[[</distalliescolor>
                        </div>
                        </div>]]end;if activeRadar.getTargetId(aW)==aW and cg<GHUD_allies_count1 then cj=cj..[[
                        ...
                        </div>]]end end end;local cC=0;local cD=0;local cE=0;if activeRadar.isConstructIdentified(aW)==1 and size~=""then local cx=activeRadar.getConstructName(aW)local cz=math.floor(activeRadar.getConstructDistance(aW))if cz>=1000 then cz=''..string.format('%0.1f',cz/1000)..'km ('..string.format('%0.2f',cz/200000)..'SU)'else cz=''..cz..'m'end;local cF=tostring(aW):sub(-3)local cG=''..cF..' '..cx..''isILock=true;cC=math.floor(activeRadar.getConstructSpeed(aW)*3.6)if activeRadar.getTargetId(aW)==aW then cm=cm..[[
                     <div class="table-row2 thS">
                     <div class="table-cellS">
                     ]]..'['..size..'] '..cG..[[ <speedcolor> ]]..cC..[[km/h</speedcolor><br><distcolor>]]..cz..[[</distcolor>
                     </div>
                     </div>]]else cm=cm..[[
                     ...
                     </div>]]end else if GHUD_show_echoes==true then if size~=""then if activeRadar.getConstructKind(aW)==5 then table.insert(radarDynamic,cw)if radarDynamicWidget[cw.widgetDist]~=nil then radarDynamicWidget[cw.widgetDist]=radarDynamicWidget[cw.widgetDist]+1 else radarDynamicWidget[cw.widgetDist]=1 end else table.insert(radarStatic,cw)if radarStaticWidget[cw.widgetDist]~=nil then radarStaticWidget[cw.widgetDist]=radarStaticWidget[cw.widgetDist]+1 else radarStaticWidget[cw.widgetDist]=1 end end end end end;if activeRadar.getThreatRateFrom(aW)~=1 and size~=""then ch=ch+1;local cx=string.sub(""..activeRadar.getConstructName(aW).."",1,11)local cz=math.floor(activeRadar.getConstructDistance(aW))if cz>=1000 then cz=''..string.format('%0.1f',cz/1000)..'km ('..string.format('%0.2f',cz/200000)..'SU)'else cz=''..cz..'m'end;local cH=tostring(aW):sub(-3)local cI=''..cH..' '..cx..''if activeRadar.getThreatRateFrom(aW)==5 then ci=ci+1;if ch<=10 then cl=cl..[[
                        <div class="table-row th">
                        <div class="lockedT">
                        <redcolor1>]]..'['..size..'] '..cI..[[</redcolor1><br><distcolor>]]..cz..[[</distcolor>
                        </div>
                        </div>]]end else if ch<=10 then cl=cl..[[
                        <div class="table-row th">
                        <div class="lockedT">
                        <orangecolor>]]..'['..size..'] '..cI..[[</orangecolor><br><distcolor>]]..cz..[[</distcolor>
                        </div>
                        </div>]]end end end;if G>50 then G=0;coroutine.yield()end end;if GHUD_show_allies==true then if cg>0 then cn="<alliescolor>Allies:</alliescolor><br><countcolor>"..cg.."</countcolor> <countcolor2>"..conID.."</countcolor2>"else cn="<alliescolor>Allies:</alliescolor><br><countcolor>0</countcolor> <countcolor2>"..conID.."</countcolor2>"end;cd=htmlbasic..[[
               <style>
               .th3>.table-cell3 {
               ...
               </div>]]end;cn="<targetscolor>Targets:</targetscolor>"cp=targetshtml..[[
            <style>
            .th2>.table-cell2 {
            ...
            <div class="table-row2 th2">
            <div class="table-cell2">
            ]]..cn..[[<br><countcolor>]]..idN-cg..[[</colorcount>
            </div>
            </div>
            ]]..cm..[[
            </div>]]if ch==0 then co="LOCK"cu="#07e88e"cs="OK"ct=cu else co="LOCKED:"cu="#FFB12C"cs=ch;ct="rgb(0, 191, 255)"end;if ci>0 then co="ATTACKED:"cu="#fc033d"cs=ci;ct="rgb(0, 191, 255)"end;cr=[[<style>.radarLockstatus {
               position: fixed;
            ...
            <rightlocked style="color: ]]..cu..[[;">]]..co..[[</rightlocked>
            </div>
            </div>
            ]]..cl..[[
            </div>]]if GHUD_show_echoes==true then local cJ=''for cv,aW in pairs(radarDynamicData)do cJ=cJ..'<span style="left:'..cv..'px;height:'..aW..'px;"></span>'end;local cK=''for cv,aW in pairs(radarStaticData)do cK=cK..'<span style="left:'..cv..'px;height:'..aW..'px;"></span>'end;local htmlRadar=htmlRadar..[[
               <div class="radar-widget">
               <div class="d-widget">]]..cJ..[[</div>
               <div class="s-widget">]]..cK..[[</div>
               <div class="labels">
               ...
               ]]radarWidget=htmlRadar else radarWidget=''end;ce=hudvers..[[<div class="hudversion">GHUD v]]..HUD_version..[[</div>]]if GHUD_show_echoes==true then if GHUD_show_allies==true then gunnerHUD=cd..cp..cq..ce..radarWidget..cr else gunnerHUD=cp..cq..ce..radarWidget..cr end else if GHUD_show_allies==true then gunnerHUD=cd..cp..cq..ce..cr else gunnerHUD=cp..cq..ce..cr end end;coroutine.yield()end end;lockhtml=[[<style>
      .table {
         display: table;
      ...
      .warp-scan {
         ...
         background: #ff3a56;
      ...
      </style>]]function zeroConvertToWorldCoordinates(d,system)local e=" *([+-]?%d+%.?%d*e?[+-]?%d*)"local f="::pos{"..e..","..e..","..e..","..e..","..e.."}"local g,h,i,j,k=string.match(d,f)if g==nil or h==nil or i==nil or j==nil or k==nil then system.print("Invalid POS!")return vec3()end;if g=="0"and h=="0"then return vec3(i,j,k)end;j=math.rad(j)i=math.rad(i)local l=atlas[tonumber(g)][tonumber(h)]local m=math.cos(i)local n=vec3(m*math.cos(j),m*math.sin(j),math.sin(i))return vec3(l.center)+(l.radius+k)*n end;function getPipeD(system)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then local aD=""local length1=-700*200000;local cL=800*200000;local cM=pos1;local cN=pos2;local pos111=zeroConvertToWorldCoordinates(cM,system)local pos222=zeroConvertToWorldCoordinates(cN,system)local DestinationCenter=vectorLengthen(pos111,pos222,length1)local DepartureCenter=vectorLengthen(pos111,pos222,cL)local cO=vec3(construct.getWorldPosition())local x=(DestinationCenter-DepartureCenter):normalize()local z=(cO-DepartureCenter):dot(x)/x:dot(x)if z<=0.then return(cO-DepartureCenter):len()elseif z>=(DestinationCenter-DepartureCenter):len()then return(cO-DestinationCenter):len()end;local A=DepartureCenter+z*x;local J=(A-cO):len()if J<1000 then aD=""..string.format("%0.0f",J).." m"elseif J<100000 then aD=""..string.format("%0.1f",J/1000).." km"else aD=""..string.format("%0.2f",J/200000).." su"end;return aD end end;function getPipeW(system)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;local length1=-700*200000;local cL=800*200000;local cM=pos1;local cN=pos2;local pos111=zeroConvertToWorldCoordinates(cM,system)local pos222=zeroConvertToWorldCoordinates(cN,system)local DestinationCenter=vectorLengthen(pos111,pos222,length1)local DepartureCenter=vectorLengthen(pos111,pos222,cL)local cO=vec3(construct.getWorldPosition())local x=(DestinationCenter-DepartureCenter):normalize()local z=(cO-DepartureCenter):dot(x)/x:dot(x)if z<=0.then return(cO-DepartureCenter):len()elseif z>=(DestinationCenter-DepartureCenter):len()then return(cO-DestinationCenter):len()end;local A=DepartureCenter+z*x;local cP="::pos{0,0,"..math.floor(A.x)..","..math.floor(A.y)..","..math.floor(A.z).."}"system.print("Pipe center")system.setWaypoint(cP)end end;function getPos4Vector(cQ)return"::pos{0,0,"..vec3(cQ).x..","..vec3(cQ).y..","..vec3(cQ).z.."}"end;function makeVector(as,at)local au=vec3(at).x-vec3(as).x;local y=vec3(at).y-vec3(as).y;local av=vec3(at).z-vec3(as).z;return vec3(au,y,av)end;function UTC()local cR=curTime-timeZone*3600;return cR end;function UTCscaner(system)local cR=system.getArkTime()-timeZone*3600;return cR end;function vectorLengthen(as,at,aw)local ax=makeVector(as,at)local ay=vec3(ax):len()local az=ay+aw;local aA=az/ay;local aB=ax*aA;local au=vec3(as).x+vec3(aB).x;local y=vec3(as).y+vec3(aB).y;local av=vec3(as).z+vec3(aB).z;local aC=vec3(au,y,av)return aC end;function start(unit,system,text)pos1time=0;pos2time=0;tspeed=0;tspeed1=0;mmode=true;deg2rad=math.pi/180;rad2deg=180/math.pi;ms2kmh=3600/1000;kmh2ms=1000/3600;showMarker=true;if GHUD_export_mode==true then system.print("---------------")system.print("The export mode is enabled ALT+G")else system.print("---------------")system.print("The export mode is disabled ALT+G")end;SU=10;calcTargetSpeed=targetSpeed/3.6;meterMarker=0;if databank_1.getStringValue(1)~=""and databank_1.getFloatValue(2)~=0 and databank_1.getStringValue(3)~=""and databank_1.getFloatValue(4)~=0 then system.print("Coordinates from DB are used!")pos1=databank_1.getStringValue(1)pos2=databank_1.getStringValue(3)pos1time=databank_1.getFloatValue(2)pos2time=databank_1.getFloatValue(4)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local dist1=pos11:dist(pos22)local cS=pos2time-pos1time;tspeed=dist1/cS;tspeed1=math.floor(dist1/cS*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew 20 km "..lengthSU1 .." su, speed "..tspeed1 .." km/h")unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)else databank_1.clear()blockTime=0;databank_1.setFloatValue(2,blockTime)databank_1.setFloatValue(4,blockTime)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;system.print("Coordinates are missing set new or export")end end;function inTEXT(unit,system,text)if pos1~=0 and string.find(text,"::pos")and not string.find(text,"m::pos")and pos2==0 and GHUD_export_mode==false then pos2=text;databank_1.setStringValue(3,pos2)pos2time=math.floor(system.getUtcTime())databank_1.setFloatValue(4,pos2time)system.print(text.." pos2 saved")pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)local dist1=pos11:dist(pos22)local cS=pos2time-pos1time;tspeed=dist1/cS;tspeed1=math.floor(dist1/cS*3.6)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;meterMarker1=meterMarker1+tspeed*4;length1=meterMarker1;resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)meterMarker=meterMarker+calcTargetSpeed*4;length=meterMarker;resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.print("---------------")system.print("The coordinates are set manually!")posExport1=databank_1.getStringValue(1)posExport2=databank_1.getStringValue(3)timeExport1=math.floor(databank_1.getFloatValue(2))timeExport2=math.floor(databank_1.getFloatValue(4))system.print("The coordinates were exported to screen")screen_1.setCenteredText(posExport1 .."/"..timeExport1 .."/"..posExport2 .."/"..timeExport2)system.print("Target speed: "..tspeed1 .." km/h")unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if pos1==0 and string.find(text,"::pos")and not string.find(text,"m::pos")and GHUD_export_mode==false then pos1=text;databank_1.setStringValue(1,pos1)pos1time=math.floor(system.getUtcTime())databank_1.setFloatValue(2,pos1time)system.print(text.." pos1 saved")end;if text=="n"then pp1=''unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;system.print("---------------")system.print("Coordinates have been deleted, set new coordinates")end;if GHUD_export_mode==true and string.find(text,"/")and not string.find(text,"/::pos")then unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;local start=0;local cT=string.find(text,"/",start)-1;pos1=string.sub(text,start,cT)system.print(pos1)start=cT+2;cT=string.find(text,"/",start)-1;pos1time=tonumber(string.sub(text,start,cT))system.print(pos1time)start=cT+2;cT=string.find(text,"/",start)-1;pos2=string.sub(text,start,cT)system.print(pos2)start=cT+2;cT=string.find(text,"/",start)pos2time=tonumber(string.sub(text,start,cT))system.print(pos2time)system.print("---------------")system.print("The coordinates have been loaded successfully!")databank_1.setStringValue(1,pos1)databank_1.setFloatValue(2,pos1time)databank_1.setStringValue(3,pos2)databank_1.setFloatValue(4,pos2time)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;oldTime=tonumber(string.sub(text,start,cT))curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local dist1=pos11:dist(pos22)local cS=pos2time-pos1time;tspeed=dist1/cS;tspeed1=math.floor(dist1/cS*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew "..lengthSU1 .." su, speed "..tspeed1 .." km/h")system.setWaypoint(Waypoint1)unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if GHUD_export_mode==true and string.find(text,"/::pos")then unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;local start=0;local cT=string.find(text,"/",start)-1;pos1=string.sub(text,start,cT)system.print(pos1)start=cT+2;cT=string.find(text,"/",start)-1;pos1time=tonumber(string.sub(text,start,cT))system.print(pos1time)start=cT+2;cT=string.find(text,"/",start)-1;pos2=string.sub(text,start,cT)system.print(pos2)start=cT+2;cT=string.find(text,"/",start)pos2time=tonumber(string.sub(text,start,cT))system.print(pos2time)system.print("---------------")system.print("The coordinates have been loaded successfully!")databank_1.setStringValue(1,pos1)databank_1.setFloatValue(2,pos1time)databank_1.setStringValue(3,pos2)databank_1.setFloatValue(4,pos2time)pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;oldTime=tonumber(string.sub(text,start,cT))curTime=system.getUtcTime()local lasttime=math.floor(curTime-pos2time)local dist1=pos11:dist(pos22)local cS=pos2time-pos1time;tspeed=dist1/cS;tspeed1=math.floor(dist1/cS*3.6)meterMarker1=lasttime*tspeed+tspeed*4;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)meterMarker=lasttime*calcTargetSpeed+calcTargetSpeed*4;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)system.print("The target flew "..lengthSU1 .." su, speed "..tspeed1 .." km/h")system.setWaypoint(Waypoint1)unit.setTimer("marker",1)unit.setTimer("vectorhud",0.02)end;if string.find(text,"mar")then if showMarker==true then showMarker=false;system.print("Current target position - OFF")end;local cU=tonumber(text:sub(4))if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then local cL=cU*200000;local cM=databank_1.getStringValue(1)local cN=databank_1.getStringValue(3)pos111=zeroConvertToWorldCoordinates(cM,system)pos222=zeroConvertToWorldCoordinates(cN,system)local cV=vectorLengthen(pos111,pos222,cL)local cW=getPos4Vector(cV)system.print(cW.." waypoint "..cU.." su")end end end;function tickVector(unit,system,text)if targetTracker==true and targetVector.x~=0 and targetVector.y~=0 and targetVector.z~=0 then local cX=getPipeD(system)local cY=vec3(construct.getWorldOrientationUp()):normalize()local cZ=vec3(construct.getWorldOrientationRight()):normalize()local c_=vec3(construct.getWorldOrientationForward()):normalize()local d0=vec3(construct.getWorldVelocity()):normalize()local d1=d0:project_on_plane(cY)local d2=d0:project_on_plane(cZ)local d3=d0:project_on_plane(c_)local d4=d3:cross(cY):normalize()myAngleR=d1:angle_between(c_)local d5=utils.sign(d4:angle_between(c_)-math.pi/2)if d5~=0 then myAngleR=myAngleR*d5;privMySignAngleR=d5 else myAngleR=myAngleR*privMySignAngleR end;local d6=d2:cross(cY):normalize()myAngleUp=d2:angle_between(-cY)-math.pi/2;local d7=utils.sign(d6:angle_between(cZ)-math.pi/2)if d7~=0 then myAngleUp=myAngleUp*d7;privMySignAngleUp=d7 else myAngleUp=myAngleUp*privMySignAngleUp end;local d8=targetVector:normalize()local d9=d8:project_on_plane(cY)local da=d8:project_on_plane(cZ)local db=d8:project_on_plane(c_)local dc=db:cross(cY):normalize()targetAngleR=d9:angle_between(c_)local dd=utils.sign(dc:angle_between(c_)-math.pi/2)if dd~=0 then targetAngleR=targetAngleR*dd;privTargetSignAngleR=dd else targetAngleR=targetAngleR*privTargetSignAngleR end;local de=da:cross(cY):normalize()targetAngleUp=da:angle_between(-cY)-math.pi/2;local df=utils.sign(de:angle_between(cZ)-math.pi/2)if df~=0 then targetAngleUp=targetAngleUp*df;privTargetSignAngleUp=df else targetAngleUp=targetAngleUp*privTargetSignAngleUp end;targetVectorWidget=[[
            ...
            .main4 {
               ...
               }     .vectorLine{position:absolute;transform-origin: 100% 0%;width: 50px;height:0.15em;}</style>]]if system.getUtcTime()-pos2time>4 then pp1=''end;vectorHUD=style..targetVectorWidget..dg end end;function tickMarker(unit,system,text)if databank_1.getStringValue(1)~=""or databank_1.getStringValue(3)~=""and databank_1.getFloatValue(2)==0 or databank_1.getFloatValue(4)==0 then pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)meterMarker1=meterMarker1+tspeed;length1=meterMarker1;lengthSU1=string.format("%0.2f",length1/200000*100/100)resultVector1=vectorLengthen(pos11,pos22,length1)Waypoint1=getPos4Vector(resultVector1)meterMarker=meterMarker+calcTargetSpeed;length=meterMarker;lengthSU=string.format("%0.2f",length/200000*100/100)resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)if showMarker==true then if mmode==true then system.setWaypoint(Waypoint1)system.print("The target flew "..lengthSU1 .." su, speed "..tspeed1 .." km/h")else system.setWaypoint(Waypoint)system.print("The target flew "..lengthSU.." su, speed "..targetSpeed.." km/h")end end end end;function altUP(unit,system,text)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;SU=SU+2.5;length=SU*200000;pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.setWaypoint(Waypoint)system.print(Waypoint.." waypoint "..SU.." su")end end;function altDOWN(unit,system,text)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;SU=SU-2.5;length=SU*200000;pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.setWaypoint(Waypoint)system.print(Waypoint.." waypoint "..SU.." su")end end;function altRIGHT(unit,system,text)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;SU=SU+10;length=SU*200000;pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.setWaypoint(Waypoint)system.print(Waypoint.." waypoint "..SU.." su")end end;function altLEFT(unit,system,text)if databank_1.getStringValue(1)~=""and databank_1.getStringValue(3)~=""then showMarker=false;SU=SU-10;length=SU*200000;pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.setWaypoint(Waypoint)system.print(Waypoint.." waypoint "..SU.." su")end end;function GEAR(unit,system,text)posExport1=databank_1.getStringValue(1)posExport2=databank_1.getStringValue(3)timeExport1=math.floor(databank_1.getFloatValue(2))timeExport2=math.floor(databank_1.getFloatValue(4))system.print("The coordinates were exported to screen")screen_1.setCenteredText(posExport1 .."/"..timeExport1 .."/"..posExport2 .."/"..timeExport2)end;function radarPos(system,dh)local bL=activeRadar.getTargetId()if bL~=0 then local cz=activeRadar.getConstructDistance(bL)local di=vec3(system.getCameraWorldForward())local dj=vec3(system.getCameraWorldPos())local dk=cz*di+dj;if pos1~=0 and pos2==0 then pos2='::pos{0,0,'..dk.x..','..dk.y..','..dk.z..'}'databank_1.setStringValue(3,pos2)pos2time=math.floor(system.getUtcTime())databank_1.setFloatValue(4,pos2time)system.print(pos2 .." pos2 saved")pos11=zeroConvertToWorldCoordinates(pos1,system)pos22=zeroConvertToWorldCoordinates(pos2,system)local dist1=pos11:dist(pos22)local cS=pos2time-pos1time;tspeed=dist1/cS;tspeed1=math.floor(dist1/cS*3.6)Pos1=pos1;Pos2=pos2;targetVector=makeVector(zeroConvertToWorldCoordinates(Pos1,system),zeroConvertToWorldCoordinates(Pos2,system))targetTracker=true;meterMarker1=meterMarker1+tspeed*4;length1=meterMarker1;resultVector1=vectorLengthen(pos11,pos22,6000000)Waypoint1=getPos4Vector(resultVector1)system.setWaypoint(Waypoint1)meterMarker=meterMarker+calcTargetSpeed*4;length=meterMarker;resultVector=vectorLengthen(pos11,pos22,length)Waypoint=getPos4Vector(resultVector)system.print("---------------")system.print("The coordinates are set manually!")posExport1=databank_1.getStringValue(1)posExport2=databank_1.getStringValue(3)timeExport1=math.floor(databank_1.getFloatValue(2))timeExport2=math.floor(databank_1.getFloatValue(4))system.print("The coordinates were exported to screen")screen_1.setCenteredText(posExport1 .."/"..timeExport1 .."/"..posExport2 .."/"..timeExport2)system.print("Target speed: "..tspeed1 .." km/h")pp1=tspeed1 ..' km/h'unit.setTimer("vectorhud",0.02)else if pos1==0 then pos1='::pos{0,0,'..dk.x..','..dk.y..','..dk.z..'}'pp1='pos1 saved'databank_1.setStringValue(1,pos1)pos1time=math.floor(system.getUtcTime())databank_1.setFloatValue(2,pos1time)system.print(pos1 .." pos1 saved")else if pos1~=0 and pos2~=0 then unit.stopTimer("marker")showMarker=true;databank_1.setStringValue(1,"")databank_1.setFloatValue(2,0)databank_1.setStringValue(3,"")databank_1.setFloatValue(4,0)pos1=0;pos2=0;lasttime=0;pos1time=0;pos2time=0;meterMarker=0;meterMarker1=0;SU=10;unit.stopTimer("vectorhud")vectorHUD=''Pos1=0;Pos2=0;privMySignAngleR=0;privMySignAngleUp=0;privTargetSignAngleR=0;privTargetSignAngleUp=0;targetVector=vec3.new(0,0,0)targetTracker=false;myAngleR=0;myAngleUp=0;targetAngleR=0;targetAngleUp=0;system.print("---------------")unit.stopTimer("vectorhud")pos1='::pos{0,0,'..dk.x..','..dk.y..','..dk.z..'}'pp1='pos1 saved'databank_1.setStringValue(1,pos1)pos1time=math.floor(system.getUtcTime())databank_1.setFloatValue(2,pos1time)system.print(pos1 .." pos1 saved")end end end end end;start(unit,system,text)local dl=system.getActionKeyName('option1')local dm=system.getActionKeyName('option2')local dn=system.getActionKeyName('option3')local dp=system.getActionKeyName('option4')local dq=system.getActionKeyName('option5')local dr=system.getActionKeyName('option6')local ds=system.getActionKeyName('option7')local dt=system.getActionKeyName('option8')local du=system.getActionKeyName('option9')local dv=system.getActionKeyName('lshift')local dw=system.getActionKeyName('gear')local dx=system.getActionKeyName('lalt')local dy=system.getActionKeyName('forward')local dz=system.getActionKeyName('backward')local dA=system.getActionKeyName('up')local dB=system.getActionKeyName('down')local dC=system.getActionKeyName('left')local dD=system.getActionKeyName('antigravity')local dE=system.getActionKeyName('right')local dF=system.getActionKeyName('yawleft')local dG=system.getActionKeyName('yawright')local dH=system.getActionKeyName('brake')local dI=system.getActionKeyName('light')local dJ=system.getActionKeyName('booster')helpHTML1=[[
         <html>
         <style>
         html,
         body {
         ...
         <br>
         <br>
         <bdr>]]..dx..[[</bdr> + <bdr>]]..dB..[[</bdr> : switch between friends/enemies<br>
         <br>
         <bdr>]]..dx..[[</bdr> + <bdr>]]..dA..[[</bdr> : construct size filter<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..dl..[[</bdr> : add/remove selected target from whitelist<br>
         </div>
         <div class="helper2">
         <ibold>TARGET VECTOR:</ibold>
         <br>
         <br>
         <bdr>]]..dw..[[</bdr> : set pos1/pos2 for radar selected target<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>↓↑</bdr> : set pos1/pos2 for radar selected target<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>←→</bdr> : move destination ±10 su<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..dx..[[</bdr> : destination to closest target pipe<br>
         <br>
         <bdr>]]..dx..[[</bdr> + <bdr>]]..dw..[[</bdr> : on/off export mode<br>
         <br>
         <bdr>]]..dJ..[[</bdr> : show/hide current target position (works only when manually setting coordinates or in export mode)<br>
         <br>
         <bdr>]]..dw..[[</bdr> + <bdr>]]..dp..[[</bdr> : switch target position between current speed or targetSpeed from LUA parameters<br>
         </div>
         </div>
         <div class="bottomL">
         <div class="helper3">
         ...
         <br>
         <br>
         <bdr>]]..du..[[</bdr> : start/stop venting<br>
         <br>
         <bdr>]]..dt..[[</bdr> : on/off shield<br>
         <br>
         <bdr>]]..ds..[[</bdr> : switch AUTO/MANUAL shield mode<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..ds..[[</bdr> : switch shield mode between MAX and 50/50 mode<br>
         <br>
         <bdr>]]..dr..[[</bdr> : agree and apply resists in manual shield mode<br>
         <br>
         <bdr>]]..dA..[[</bdr> + <bdr>]]..dl..[[</bdr> : 100% antimatter power<br>
         <br>
         <bdr>]]..dA..[[</bdr> + <bdr>]]..dm..[[</bdr> : 100% electromagnetic power<br>
         <br>
         <bdr>]]..dA..[[</bdr> + <bdr>]]..dn..[[</bdr> : 100% thermic power<br>
         <br>
         <bdr>]]..dA..[[</bdr> + <bdr>]]..dp..[[</bdr> : 100% kinetic power<br>
         <br>
         <bdr>]]..dB..[[</bdr> + <bdr>]]..dl..[[</bdr> : cannon profile<br>
         <br>
         <bdr>]]..dB..[[</bdr> + <bdr>]]..dm..[[</bdr> : laser profile<br>
         <br>
         <bdr>]]..dB..[[</bdr> + <bdr>]]..dn..[[</bdr> : railgun profile<br>
         <br>
         <bdr>]]..dB..[[</bdr> + <bdr>]]..dp..[[</bdr> : universal profile<br>
         </div>
         <div class="helper2">
         <ibold>Other:</ibold>
         <br>
         <br>
         <bdr>]]..dH..[[</bdr> + <bdr>]]..dv..[[</bdr> : lock brake<br>
         <br>
         <bdr>]]..dl..[[</bdr> : show/hide planets and planetary periscope<br>
         <br>
         <bdr>]]..dm..[[</bdr> : set destination to planet #1 (closest pipe planets)<br>
         <br>
         <bdr>]]..dn..[[</bdr> : set destination to closest pipe<br>
         <br>
         <bdr>]]..dp..[[</bdr> : set destination to planet #2 (closest pipe planets)<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..dm..[[</bdr> : set destination to destination planet (LUA parameters)<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..dn..[[</bdr> : set destination to custom pipe Destination - Departure (LUA parameters)<br>
         <br>
         <bdr>]]..dv..[[</bdr> + <bdr>]]..dp..[[</bdr> : set destination to departure planet (LUA parameters)<br>
         <br>
         <bdr>]]..dq..[[</bdr> : Helios system map<br>
         <br>
         <bdr>]]..dI..[[</bdr> : set destination to saved position<br>
         </div>
         </div>
         <div class="bottomL">
         <div class="helper3">
